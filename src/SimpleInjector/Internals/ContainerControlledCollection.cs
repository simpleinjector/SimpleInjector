...
namespace SimpleInjector.Internals
{
        ...
        IContainerControlledCollection, IReadOnlyList<TService>
    {
        ...
        public virtual IEnumerator<TService> GetEnumerator()
        {
            var producers = this.producers.Value;

            // PERF: Empty collection causes no garbage when iterated.
            return producers.Length == 0 ? EmptyEnumerator.Instance : new Enumerator(this.producers.Value);
        }
        ...
        private InstanceProducer GetOrCreateInstanceProducer(ContainerControlledItem item)
        {
            ...
            {
                // Something special just happened. The user registered an item as part of the collection
        ...
        private sealed class EmptyEnumerator : IEnumerator<TService>
        {
            public static readonly IEnumerator<TService> Instance = new EmptyEnumerator();

            private EmptyEnumerator() { }

            public TService Current { get; } = default!;
        ...
        private sealed class Enumerator : IEnumerator<TService>
        {
            ...
            private int index;
            private TService current = default!;
